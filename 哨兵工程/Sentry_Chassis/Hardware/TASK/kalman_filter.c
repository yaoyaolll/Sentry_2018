...
//原inv函数有问题，会改变输入mat的值，2*2 matrix
void mat_inv(arm_matrix_instance_f32 * pSrc, 
  arm_matrix_instance_f32 * pDst)
{
	float _pSrc[4] = {0, 0, 0, 0};
	int index;
	for(index = 0;index < 4;index ++)
		_pSrc[index] = pSrc->pData[index];     //保存原始指针指向的值
	
	_mat_inv(pSrc, pDst);                    //矩阵倒数运算
	
	for(index = 0;index < 4;index ++)
		pSrc->pData[index] = _pSrc[index];
}
...
float *kalman_filter_calc(kalman_filter_t *F, float signal1, float signal2)  //signal为实际测量值
{
